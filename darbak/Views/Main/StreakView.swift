...
struct StreakView: View {
            ...
            VStack(spacing: 15) {
              ...
              LazyVGrid(
                ...
                StreakStatCard(
                  ...
                  subtitle: "هدف اليوم",
                  icon: "target",
                  color: getProgressPercentage() >= 100 ? DesignSystem.Colors.success :
  ...
  private func loadWeeklyData() {
    ...
    // Adjust the week start based on selectedWeek
    guard let weekStartOfToday = calendar.dateInterval(of: .weekOfYear, for: today)?.start else {
      return
    }
    guard let selectedWeekStartDate = calendar.date(
      byAdding: .weekOfYear,
      value: selectedWeek,
      to: weekStartOfToday
    ) else { return }
    ...
    weeklyData = (0..<7).compactMap { dayOffset in
      guard let date = calendar.date(byAdding: .day, value: dayOffset, to: selectedWeekStartDate)
      else { return nil }
    ...
    // If we have HealthKit access, try to fetch historical data for the selected week
  ...
  private func fetchWeeklyDataFromHealthKit() {
    ...
    guard let selectedWeekEndDate = calendar.date(
      byAdding: .day,
      value: 7,
      to: selectedWeekStartDate
    ) else { return }

    healthKitManager.fetchStepsForDateRange(from: selectedWeekStartDate, to: selectedWeekEndDate) {
      stepsData in
  ...
  private func getStepsForDate(_ date: Date) -> Int {
    if Calendar.current.isDateInToday(date) && selectedWeek == 0 {
      return getCurrentSteps()
    } else {
      // Get historical data from StreakManager
      // Note: The key should be based on the date itself, not timeIntervalSince1970
      // if you want to be precise about getting data for the exact day.
      let dateFormatter = DateFormatter()
      dateFormatter.dateFormat = "yyyy-MM-dd"
      let dateString = dateFormatter.string(from: date)
      let key = "steps_\(dateString)"
